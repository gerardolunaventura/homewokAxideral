...
JSUtil = function(){
	this.tagsIgnored = ["option", "head", "script", "title", "link", "noscript", "style", "meta" ];
};
...
JSUtil.prototype={
		...
		isDragEvent: function(event, eventName, triggeredEvent) {
			if (event && eventName == triggeredEvent && rmotRecorder.downEvent) {
				...
				if (Math.abs(rmotRecorder.downEvent.x - upEvent.x) > deviation
						|| Math.abs(rmotRecorder.downEvent.y - upEvent.y) > deviation) {
		 ...
		 */
		getWindowWidth:function() {//RMoTgetWindowWidth	
			try { return window.innerWidth; } catch(err){
				try { return document.documentElement.clientWidth; } catch (err) {
					try { return document.body.clientWidth;} catch(err) { return 0;
					}
				}
			}
		},

		getWindowHeight:function() {//RMoTgetWindowHeight
			try { return window.innerHeight; } catch(err){
				try { return document.documentElement.clientHeight; } catch (err) {
					try { return document.body.clientHeight;} catch(err) { return 0;
					}
				}
			}
		},

		getScrollLeft:function() {//RMoTgetScrollLeft
			try { return window.pageXOffset; } catch(err){
				try { return document.documentElement.scrollLeft; } catch (err) {
					try { return document.body.scrollLeft;} catch(err) { return 0;
					}
				}
			}
		},

		getScrollTop:function() {//RMoTgetScrollTop
			try { return window.pageYOffset; } catch(err){
				try { return document.documentElement.scrollTop; } catch (err) {
					try { return document.body.scrollTop;} catch(err) { return 0;
					}
				}
			}
		},
		...
		isInViewPort:function (element, rect) {//RMoTisInViewPort
			...
			if ( rect.right < 0  // element on the left
				|| rect.left > this.getWindowWidth()  // element on the right
				|| rect.bottom < 0  // element above
				|| rect.top > this.getWindowHeight() )  // element below
					{
		...
		isOnTop:function (element) {//RMoTisOnTop
			...
			var b = {	relativeLeft: Math.min(rect.right - 1, this.getWindowWidth()  - 1), 
						relativeTop: a.relativeTop };

			var c = {	relativeLeft: a.relativeLeft, 
						relativeTop:  Math.min(rect.bottom - 1, this.getWindowHeight() - 1) };
		...
		getXPath:function (node) {//RMoTgetXPath (only used for debugging purposes)
			...
			do {
				...
				fullPath.unshift(el.nodeName.toLowerCase() + (id ? '[@id=\'' + id + '\']' : '') + (index > 1 ? '[' + index + ']' : ''));
				
			} while (!id && ((el.nodeName.toLowerCase() != 'body') && el.parentNode && (el = el.parentNode)));
		...
		setOwnerDocumentCoordinates:function(element) {
			...
			var innerCoords = {};
			innerCoords.left = rect.left + ownerRect.left;
			innerCoords.top = rect.top + ownerRect.top;
			innerCoords.right = rect.left + ownerRect.right;
			innerCoords.bottom = rect.top + ownerRect.bottom;
		...
		getCoordinatesArray:function(proxy) {
			...
			return "left:"+Math.floor(rect.left)
		...
		getComputedStyle: function(element) {
			...
			if (window.getComputedStyle) {
				...
				for (var i = 0; i < RMoTstylesArray.length; i++) {
					...
					var ret = jsUtil.getMainDocument().defaultView.getComputedStyle(element);
					if (ret != null) {
		...
		getBrowserCompatibilityDetails: function() {
			var ret = {status: true, name: "", version: 1, documentMode:"", compatMode: "Standards", tracemode: true};

			var navAgent = navigator.userAgent;
			var Version;	
			if ((Version = navAgent.indexOf("MSIE"))!=-1) {
			...
			else if ((Version=navAgent.indexOf("Chrome"))!=-1) {
				postXHRThroughCustomEventFlag = true;
				ret.name = "Chrome";
				ret.version = parseFloat(navAgent.substring(Version+ 7));
				BROWSER_ZOOM_FACT = window.devicePixelRatio;
			}
			// In Safari, the true version is after "Safari" or after "Version" 
			else if ((Version=navAgent.indexOf("Safari"))!=-1) {
				postXHRThroughCustomEventFlag = true;
				ret.name = "Safari";
				Version = navAgent.substring(Version+7);
			...
			else if ((Version=navAgent.indexOf("Firefox"))!=-1) {
				postXHRThroughCustomEventFlag = true;
				ret.name = "Firefox";
			...
			//alert(ret.name + " : " + ret.version + " : " + ret.documentMode);
			return ret;
...
DomainManager.prototype={
		...
		getProxy: function(element){
			...
			if (!element.registeredClassName || element.registeredClassName != element.className) {
	...
	if (element) {
		...
		if (rect) {
			...
			if (!jsUtil.isInViewPort(element, rect)) {			
				var scrollX = (coords.right > jsUtil.getWindowWidth()) ? coords.right - jsUtil.getWindowWidth() + 100 : 0;
				var scrollY = (coords.bottom > jsUtil.getWindowHeight()) ? coords.bottom - jsUtil.getWindowHeight() + 100 : 0;
...
RMoTRectangle.prototype.getCoords = function(element) {
	...
	if (element != null) {
		...
		// Specific to Dojo domain as scroll actions are recorded
		if (typeof(DOJO_DOMAIN_NAME)!='undefined' && domainManager.isRegistered(DOJO_DOMAIN_NAME)) {
			ret = proxy.getCoordinates();	
		}
	}
	return ret;
...
RMoTRectangle.prototype.drawRectangle = function(doc, top, left, width, height, color) {
	...
	this.RMoTRect = doc.createElement('div');
	...
	if (element && element.tagName && 
		...
		try {
			...
			if(typeof(crossScriptIFrames) != 'undefined' && crossScriptIFrames != null) {
				crossScriptIFrames.push(element); //collecting the cross scripting frames/iframes
			}
		}
		finally {
...
HtmlElementProxy.prototype.getLabelText = function (tentativeLabel,lbl,trav) {
	...
	if(tentativeLabel.contains(this.element)){
...
HtmlElementProxy.prototype.installDownEventWrapper = function (eventName) {
...
HtmlElementProxy.prototype.getEventsToRegister = function() {
	return [RMOT_TRIGGER_EVENT, RMOT_DBLCLICK_EVENT];
};

HtmlElementProxy.prototype.shouldIgnoreEvent = function(element, event) {

	if (!event.disableRecordAll && (window.RMoTisListened || document.RMoTisListened || recordAll == RECORD_ALL_ENABLED)) return false; // 47726

	var proxy = domainManager.getProxy(element);

	for (var e in element.rmotOriginalHandler) {
		if (element.rmotOriginalHandler[e]!=null) {
			if (jsUtil.getClickEvents().indexOf(e) >= 0) {
				return false;
			}
		}
	}

	if (element.RMoTisListened) {
		return false; // Listeners added thanks to addEventListener
	}

	// Check for listeners on alternate click events
	var alternateClickEvents = jsUtil.getAlternateClickEvents();
	for (var i = 0; i < alternateClickEvents.length; i++) {
		var evt = alternateClickEvents[i];
		if (element[evt]) {
			return false;
		}
	}
...
HtmlElementProxy.prototype.handleEvent = function(element, eventName, event) {
	if (eventName == RMOT_TRIGGER_EVENT) {
		...
		if (domainManager.getProxy(element).shouldIgnoreEvent(element, event)) {
			// Dump repeated action if any
...
HtmlElementProxy.prototype.isDblClickHandled = function(eventName, event) {
	var ret = false;
	var elt = this.element;
	while(elt != null) {
		if ((elt.rmotOriginalHandler[RMOT_DBLCLICK_EVENT] != null || elt.RMoTdblclickIsListened)) {
			if (eventName != RMOT_DBLCLICK_EVENT) { event.recorded = true; return true; }
			else return false;
		}
		else {
			ret = (eventName == RMOT_DBLCLICK_EVENT) ? true : false;
		}
		elt = elt.parentElement;
	}
	
	if (ret) event.recorded = true;
	return ret;
};

HtmlElementProxy.prototype.getComputedStyle = function(propName) {
	var computedStyle = "";
	var mainWindow = jsUtil.getMainDocument().defaultView;
	if (mainWindow.getComputedStyle) {
...
HtmlElementProxy.prototype.getProperty = function(propName) {
	...
	if (propName == WebGuiConstants.TAGNAME_PROP) {
		...
		propValue = jsUtil.getTrimText(this.element);
	}
...
HtmlElementProxy.prototype.getCoordinates =  function() {
	...
	if (element.nodeType == window.Node.ELEMENT_NODE) {
		...
		if (rect && element.nodeType) {
			...
			return {left:BROWSER_ZOOM_FACT*Math.floor(ownerRect.left+rect.left),
				top:BROWSER_ZOOM_FACT*Math.floor(ownerRect.top+rect.top),
				right:BROWSER_ZOOM_FACT*Math.floor(ownerRect.right+rect.right),
				bottom:BROWSER_ZOOM_FACT*Math.floor(ownerRect.bottom+rect.bottom)};
		}	
	}
		...
		if (this.getComputedStyle('display') == 'none') { 
			...
			if (this.getComputedStyle('visibility') == 'hidden') {
				...
				if (this.element.offsetWidth === 0 ||
					...
					// Check whether the element is underneath another element
					if (!jsUtil.isOnTop(this.element)) {
						ret.visibility = false;
					}
				}
			}
...
HtmlElementProxy.prototype.change = function(value) {
	...
	for (var j = 0; j < this.target.options.length; j++) {
		...
		if (this.target.options[j].selected !== selected) {
...
HtmlElementProxy.prototype.executeAction = function(action) {
	...
	if (typeof(this.target.focus) == "function") {		// WI 66075 it seems that calling the focus() function leads to unexpected behaviour with some frameworks
		this.dispatchInputEvent('focus');
	}
...
HtmlEditableProxy.prototype.handleEvent = function(element, eventName, event) {
	if (eventName == RMOT_TRIGGER_EVENT && this.isDblClickHandled(eventName, event)) return;
...
HtmlSelectProxy.prototype.getEventsToRegister = function() {
	return ["onchange", RMOT_DOWN_EVENT];
};
...
HtmlSelectProxy.prototype.installDownEventWrapper = function (eventName) {};
...
HtmlSelectProxy.prototype.handleEvent = function(element, eventName, event) {
	...
	if (eventName == "onchange" && this.selectopen) {
		...
		this.domainObj._recordEventWithExistingScreenshot(eventName, this.getProperty(WebGuiConstants.TAGNAME_PROP), 
			...
			this.getParameters(), this.domainObj._captureScreenshot());
		this.selectopen = false;
	}
	else {
		this.selectopen = true;
		webGuiRecorderInterfaceObj.handleRepeatedAction(element, eventName); // Dump running repeated action if any
...
HtmlTextInputProxy.prototype.getLabel = function () {
	//Compute label from the placeholder
	var label = this.getProperty('placeholder');
	label = label.trim();
	if(typeof(label)=='undefined' || label == null || label == ""){
		var direction = traversal.LEFT;
		if( jsUtil.isRTLLanguage() == true){
...
HtmlInputFileProxy.prototype.executeAction = function(action) {
	if (action.type=='onclick' && jsUtil.isRunningInsideIEBrowser()) {	
		setTimeout(function() {
			this.setTarget(this.element);
			this.click();
		}, 0);
...
HtmlAreaProxy.prototype.getCoordinates = function() {
	...
	if (element.nodeType == window.Node.ELEMENT_NODE) {
		if (element.nodeType) {
			...
			return {left:Math.floor(ownerRect.left+rect.left),
					top:Math.floor(ownerRect.top+rect.top),
					right:Math.floor(ownerRect.right+rect.right),
					bottom:Math.floor(ownerRect.bottom+rect.bottom)}
		}	
	}
...
HtmlBodyProxy.prototype.getProperty = function(propertyName) {
	...
	switch (propertyName) {
		...
		case WebGuiConstants.WIDTH_PROP:
			propertyValue = jsUtil.getWindowWidth();
			break;
		case WebGuiConstants.HEIGHT_PROP:
			propertyValue = jsUtil.getWindowHeight();
			break;
		case WebGuiConstants.SCROLLABLE_PROP:
			propertyValue = (document.body.scrollHeight > jsUtil.getWindowHeight()) ? true : false;
			break;
		default:
			propertyValue = this._getProperty(propertyName);
	}
...
HtmlFrameProxy.prototype.isVisible = function() {
	...
	if (!ret.visibility) ret.propagation = true;
	return ret;
};
...
JQueryUIElementProxy.prototype._shouldIgnoreEvent=JQueryUIElementProxy.prototype.shouldIgnoreEvent;
JQueryUIElementProxy.prototype.shouldIgnoreEvent = function(element, event) {
	if (jqueryUtil.isClickEventHandler(element)) return false; // 51477
	
	return _shouldIgnoreEvent(element, event);
};
////////////////////////////////// JQueryUIElementProxy //////////////////
...
JQueryMobileElementProxy.prototype.handleEvent = function(element, eventName, event) {
	...
	// Log event only if there is a listener on the original app
	if (proxy.shouldIgnoreEvent(element, event)) {
		return;
	}
...
JQueryUIButtonProxy.prototype.handleEvent = function(element,methodName,event){
	...
	// get the class list of the element to check the ignored class(es)
	var classNames = webGuiUtil.jQuery(element).attr("class").split(' ');
	// TODO make the ignore list more generic
	if (classNames && classNames.indexOf("ui-dialog-titlebar-close") > -1) {
		return;
	}	
	var parameters = {};
	var eventName = "onclick";
...
JQueryUIButtonProxy.prototype._JQWidgetinstallWrappers = JQueryUIButtonProxy.prototype.installWrappers;
JQueryUIButtonProxy.prototype.installWrappers = function(){
	if (this.element && this.element.nodeType == window.Node.ELEMENT_NODE) {
		var elementclassName = this.element.className;
		if(elementclassName.indexOf("ui-dialog-titlebar-close") == -1){
			return this._JQWidgetinstallWrappers();
		}
		return true; // Continue with ELEMENT_NODE children
	}
	return false; // Don't visit known ELEMENT_NODE children return true;
...
JQueryUIDateProxy.prototype.installWrappers = function(){
	if (this.element && this.element.nodeType == window.Node.ELEMENT_NODE) {
		return true; // Continue with ELEMENT_NODE children
	}
	return false; // Don't visit known ELEMENT_NODE children return true;
...
/*		var closebutton = this.getDialogCloseButton();
		if(closebutton != null){
			this.interceptEvent( closebutton, "onclick", this.handleEvent);
		}*/
		return true; // Continue with ELEMENT_NODE children
	}
...
JQueryUIDialogProxy.prototype.getDialogCloseButton = function( ){
	var closeButton =null;
	if (window.jQuery) {
		var jqWidget = webGuiUtil.jQuery(this.element);
		var elemClass = jqWidget.attr('class');
		var results = null;
		// if the element in content is the dialog content, 
		// get the dialog object (parent) to find the 'close' button
		// otherwise execute the 'find' on 'dialog' object directly 
		if (elemClass && elemClass.indexOf("ui-dialog-content") >= 0) {			
			results = jqWidget.parent().find("*[class~=ui-dialog-titlebar-close]");			
		} else  {
			results = jqWidget.find("*[class~=ui-dialog-titlebar-close]");
		}
		if (results && results.length > 0) {
			closeButton = results[0];
		}
	}
	if(closeButton == null){
		//console.log("RTW_WEBGUI_ERR:Unable to find native input element for ", this.element);
...
JQueryUIDialogProxy.prototype.executeAction = function(action) {
	...
	case "onclose" :
		var closebutton = this.getDialogCloseButton();
		if(closebutton != null){
			this.setTarget(closebutton);
			this.click();
			retStatus = RMOT_SUCCESS;
		}else{
			LoggerService.logMsg(RMoTTrace, "Error: Close button not available on Dialog");
		}
		break;
	default:
		retStatus = this._JQWidgetexecuteAction(action);
...
JQueryUISelectmenuProxy.prototype.handleJQSelectmenuEvent = function( event, ui, element ) {
	...
	case "selectmenuopen":
    	...
    	if(jqueryproxy.started == 0){
    		jqueryproxy.screenshotid = jqueryproxy.domainObj._captureScreenshot();
    		jqueryproxy.hierarchy = jqueryproxy.domainObj._updateHierarchy(event.target);
    		jqueryproxy.started = 1;
    	}
		break;
	case "selectmenuchange":
		...
		jqueryproxy.domainObj._recordEventWithExistingScreenshot(eventName, tagName, element, rmotHierarchy.currentHierarchy, parameters, jqueryproxy.screenshotid);
		jqueryproxy.hierarchy = null;
		event.recorded = true;
		jqueryproxy.screenshotid = null;
		jqueryproxy.started = 0;
	...
	case "onchange" :
		try {
			...
			if(selectElement && dropdownMenuElement) {
				...
				var optionElement = webGuiUtil.jQuery(dropdownMenuElement).children().filter(function() {return webGuiUtil.jQuery(this).text().trim() == option;});
				
				if(optionElement) {
					webGuiUtil.jQuery(dropdownMenuElement).menu("focus",null, 
							webGuiUtil.jQuery(dropdownMenuElement).find(optionElement)).menu("select");
...
WebGuiRecorderInterface.prototype ={
		...
		captureScreenshot:function(){
			...
			if(jsUtil.isDesktop()){
				...
				rmotRecorder.postXMLHttpRequest('/RTWWebGui/Snapshot', JSON.stringify ( { actionId: actionId ,snapshotRect: jsUtil.getSnapShotRect(),tagName: 'BROWSER', snapshot : true } ), false);
			}
			return actionId;
		},
		...
		recordEventWithExistingScreenshot:function(eventName, tagName, element, hierarchy, parameters, actionId){
			...
			if( (jsUtil.isDesktop()) && 
				(typeof isDesktopRecorderActive != undefined) && (isDesktopRecorderActive === false) ) {
				event.recorded = true; 
				return;
			}
		...
		recordEvent:function(event, eventName, tagName, element, parameters){
			...
			if (proxy.getUID()!='undefined') {
				var jsonString = rmotRecorder.buildJsonString(eventName, tagName, element, hierarchy, parameters, actionId);
				this.webGuiRecorderObj.logEvent(jsonString);
...
WebGuiRecorder.prototype ={
		...
		addAjaxListener:function() {//RMoTAddAjaxListener
			...
			if (typeof(xmlRequest.rmotOpen)==='undefined') {
				...
				xmlRequest.open = function() {
					this.addEventListener("load", ajaxParse, false);
			...
			if(jsUtil.isDesktop()) {
				...
				window.document.onreadystatechange = function () {
					  if(document.readyState === "complete"){
						  ...
						  rmotRecorder.postXMLHttpRequest('/RTWWebGui/DocReady', JSON.stringify ( { eventName: 'DOCREADY',tagName: 'BROWSER',timestamp : startTime } ), false);
					  }
				};
				...
				window.onbeforeunload=function(){
					if(window.unload == undefined) {
						...
						rmotRecorder.postXMLHttpRequest('/RTWWebGui/DocUnload', JSON.stringify ( { eventName: 'UNLOAD',tagName: 'BROWSER' } ) , true);
					}
				};
		...
		visit:function (node, func) {//RMoTvisit	
			...
			if (node && (node.nodeType == window.Node.ELEMENT_NODE || node == document)) {
		...
		installWrappers: function(node) {
			if (node.tagName && jsUtil.tagsIgnored.indexOf(node.tagName.toLowerCase()) < 0) {
				...
				domainManager.installDownWrapper(node);
		...
		buildJsonStringMobile:function(eventName, tagName, element, hierarchy, parameters) {//RMoTbuildJsonString
			...
			return JSON.stringify({ eventName: eventName, 
									...
									scrollLeft: jsUtil.getScrollLeft(),
									...
									webviewSettings: { 	width:jsUtil.getWindowWidth(), 
														height:jsUtil.getWindowHeight() }	
									});
		},
		...
		buildJsonStringDesktop : function(highlightUnsupported, eventName, tagName, element, hierarchy, parameters, actionId, scrollLeft, scrollTop, snapshotRect,
			...
			return JSON.stringify({
				...
				scrollLeft : (scrollLeft == undefined ? jsUtil.getScrollLeft() : scrollLeft),
				scrollTop : (scrollTop == undefined ? jsUtil.getScrollTop() : scrollTop),
		   ...
		   if(xmlHttp)
		   {
			   ...
			   xmlHttp.onreadystatechange = function() {
				   if (xmlHttp.readyState == 4) {
					   rmotRecorder.updatepage(xmlHttp.status);
				   }
			   };
			   xmlHttp.send(value);
		   }
...
WebGuiAction.prototype={
	...
	isRepeated: function (proxy, eventName) {
		// return (proxy.getRepeatedEvent() == eventName);
...
WebGuiHierarchy.prototype ={
	...
	getHierarchy: function() {
		...
		// rmotRecorder.log(JSON.stringify(this.currentHierarchy));
	...
	cloneElementAndChildren: function(proxy) {
		...
		clone.xpathProp = proxy.getProperty(WebGuiConstants.XPATH_PROP);
...
var browserDetails = {status: true, name: "RTWBrowser", version: 1, documentMode:"", tracemode: false};
...
var RECORD_ALL_DISABLED = 0;
var RECORD_ALL_ENABLED = 1;

var recordAll = RECORD_ALL_DISABLED;
